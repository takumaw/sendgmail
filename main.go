// Copyright 2025 Takuma Watanabe.
// Based on the work by Google Inc.
//
...
func sendMessage(config *oauth2.Config) {
	...
	gmailService, err := gmail.NewService(context, option.WithTokenSource(config.TokenSource(context, &token)))
	if err != nil {
	...
	gmailMessage.Raw = base64.URLEncoding.EncodeToString(message)
	_, err = gmailService.Users.Messages.Send(sender, &gmailMessage).Do()
	if err != nil {
